...
export class AppComponent {
  ...
  navName: string[] = ['Dashboard', 'Task', 'Add'];
  navSrcImg: string[] = [
  ...
  get navFocus() {
    return this.userService.navFocus;
  }

  changeNav(value: string) {
    this.userService.navFocus = value;
  ...
  inroom(i: number): boolean {
    const room = this.userService.room_chat?.member_id;
    if (!room) return false;
    return room.split(',').includes(i.toString());
  }

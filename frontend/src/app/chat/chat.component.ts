...
export class ChatComponent {
  ...
  searchusername = '';
  getUser() {
    const url = `http://localhost:4000/user?userid=${this.userService.user?.userid}&username=${this.searchusername}`;
    fetch(url, {
      method: 'GET',
      headers: {
  ...
  getRequest() {
    const url = `http://localhost:4000/request?userid=${this.userService.user?.userid}`;
  ...
  sendfriendrequest(friend: {
    ...
    fetch('http://localhost:4000/request/friend', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
  ...
  async getFriend() {
    try {
      const res = await fetch(
        `http://localhost:4000/friend?userid=${this.userService.user?.userid}`,
        {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
